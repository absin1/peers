...
public class MediaManager {
	...
	private void startRtpSessionOnSuccessResponse(String localAddress, String remoteAddress, int remotePort,
		...
		rtpSession = new RtpSession(inetAddress, datagramSocket, userAgent.isMediaDebug(), logger,
		...
		try {
			captureRtpSender = new CaptureRtpSender(rtpSession, soundSource, userAgent.isMediaDebug(), codec, logger,
					userAgent.getPeersHome());
		} catch (IOException e) {
	...
	public void successResponseReceived(String localAddress, String remoteAddress, int remotePort, Codec codec) {
		...
		case captureAndPlayback:
			...
			soundManager.init();
			startRtpSessionOnSuccessResponse(localAddress, remoteAddress, remotePort, codec, soundManager);
		...
		case echo:
			Echo echo;
			try {
				echo = new Echo(datagramSocket, remoteAddress, remotePort, logger);
			} catch (UnknownHostException e) {
				logger.error("unknown host amongst " + localAddress + " or " + remoteAddress);
				return;
			}
		...
		case file:
			...
			try {
				incomingRtpReader = new IncomingRtpReader(captureRtpSender.getRtpSession(), null, codec, logger);
			} catch (IOException e) {
				logger.error("input/output error", e);
				return;
			}
	...
	public void handleAck(String destAddress, int destPort, Codec codec) {
		...
		case captureAndPlayback:
			...
			try {
				...
				incomingRtpReader = new IncomingRtpReader(rtpSession, soundManager, codec, logger);
			} catch (IOException e) {
				logger.error("input/output error", e);
				return;
			}
		...
		case echo:
			...
			try {
				...
				logger.error("unknown host amongst " + userAgent.getConfig().getLocalInetAddress().getHostAddress()
						+ " or " + destAddress);
				return;
			}
			userAgent.setEcho(echo);
		...
		case file:
			...
			// String fileName = userAgent.getConfig().getMediaFile();
			String fileName = "C:\\Users\\absin\\Downloads\\output.wav";
			fileReader = new FileReader(fileName, logger);
			startRtpSession(destAddress, destPort, codec, fileReader);
			try {
				incomingRtpReader = new IncomingRtpReader(rtpSession, null, codec, logger);
			} catch (IOException e) {
				logger.error("input/output error", e);
				return;
			}
